...
const SORT_COLS = ['name', 'date', 'size', 'type'] as const;
...
export default function ListView() {
  ...
  const items = (() => {
    ...
    const comparator = (a: DriveItem, b: DriveItem) => {
      let cmp = 0;
      switch (sortBy) {
        case 'name':
        ...
        case 'type': {
          const ta = getFileTypeLabel(a.mimeType);
          const tb = getFileTypeLabel(b.mimeType);
  ...
  const handleRowDoubleClick = useCallback((item: DriveItem) => {
    if (item.isFolder) {
            ...
            {SORT_COLS.map((col) => {
              ...
              return (
                <th
                  key={col}
                  onClick={() => setSort(col)}
                  className="px-4 py-2.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider cursor-pointer select-none hover:text-gray-700 motion-safe:transition-colors"
                >
                  <div className="flex items-center gap-1">
                    {SORT_LABELS[col]}
                    {isActive && (
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" width="12" height="12" className="text-blue-500 shrink-0">
                        {sortDir === 'asc' ? (
          ...
          {items.map((item) => {
            ...
            return (
              <tr
                ...
                className={`border-b border-gray-100 motion-safe:transition-colors cursor-pointer active:scale-[0.97] ${
                  isSelected
              ...
              >
                <td className="px-4 py-2.5">
                  <div className="flex items-center gap-2.5">
                    <span
                      className={`shrink-0 ${item.isFolder ? 'text-amber-500' : 'text-gray-400'}`}
